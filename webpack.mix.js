...
let assetSourceDemo = demo === 'absensi' ? 'demo1' : demo;
...
['public/css', 'public/js', 'public/media', 'public/plugins'].forEach(dir => {
...
mix.sass(`resources/assets/core/plugins/plugins.scss`, `public/${demo}/plugins/global/plugins.bundle.css`).then(() => {
    ...
    if (fs.existsSync(path.resolve('public/fonts'))) {
        fs.rmSync(path.resolve('public/fonts'), { recursive: true, force: true });
    }
    if (fs.existsSync(path.resolve('public/images'))) {
        fs.rmSync(path.resolve('public/images'), { recursive: true, force: true });
    }
...
mix.sass(`resources/assets/${assetSourceDemo}/sass/plugins.scss`, `public/${demo}/plugins/global/plugins-custom.bundle.css`);

// Build Metronic css/js
mix.sass(`resources/assets/${assetSourceDemo}/sass/style.scss`, `public/${demo}/css/style.bundle.css`, {sassOptions: {includePaths: ['node_modules']}})
    // .options({processCssUrls: false})
    .scripts(require(`./resources/assets/${assetSourceDemo}/js/scripts.js`), `public/${demo}/js/scripts.bundle.js`);
...
if (args.indexOf('dark_mode') !== -1) {
    ...
    mix.sass(`resources/assets/${assetSourceDemo}/sass/plugins.dark.scss`, `public/${demo}/plugins/global/plugins-custom.dark.bundle.css`);
    mix.sass(`resources/assets/${assetSourceDemo}/sass/style.dark.scss`, `public/${demo}/css/style.dark.bundle.css`, {sassOptions: {includePaths: ['node_modules']}});
}
...
(glob.sync(`resources/assets/${assetSourceDemo}/sass/pages/**/!(_)*.scss`) || []).forEach(file => {
    file = file.replace(/[\\\/]+/g, '/');
    mix.sass(file, file.replace(`resources/assets/${assetSourceDemo}/sass`, `public/${demo}/css`).replace(/\.scss$/, '.css'));
});
...
mix.copyDirectory(`resources/assets/${assetSourceDemo}/media`, `public/${demo}/media`);

// Metronic theme
(glob.sync(`resources/assets/${assetSourceDemo}/sass/themes/**/!(_)*.scss`) || []).forEach(file => {
    file = file.replace(/[\\\/]+/g, '/');
    mix.sass(file, file.replace(`resources/assets/${assetSourceDemo}/sass`, `public/${demo}/css`).replace(/\.scss$/, '.css'));
});
...
function getDemos(pathDemos) {
    ...
    let demos = [];
    args.forEach((arg) => {
        const demo = arg.match(/^(demo.*|absensi)/g);
        if (demo) {
            demos.push(demo[0]);
        }
    });
