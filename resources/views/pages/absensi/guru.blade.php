...
@if(session('delete_success'))
    <div class="alert bg-light-danger border border-danger alert-dismissible fade show p-5 mb-10 position-relative">
        <div class="d-flex align-items-center mb-3 pe-8">
            ...
            <div class="d-flex flex-column flex-grow-1">
                <h4 class="mb-1 text-danger">Hasil Penghapusan Data Guru</h4>
                <span class="text-gray-700 fs-6">
                    Sebanyak <strong class="text-danger">{{ session('delete_success')['count'] }} data guru</strong> telah berhasil dihapus secara permanen dari database.
                </span>
            </div>
        </div>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        ...
        @if(!empty(session('delete_success')['items']))
            <div class="mt-4 pt-3 border-top border-danger border-opacity-25">
                <a class="btn btn-sm btn-light-danger fw-bolder mb-2" data-bs-toggle="collapse" href="#collapseGuruDeleted" role="button" aria-expanded="false">
                    <i class="bi bi-trash me-1"></i> Lihat Daftar Guru yang Dihapus ({{ session('delete_success')['count'] }} Guru)
                </a>
                <div class="collapse show" id="collapseGuruDeleted">
                    <div class="card card-body bg-white border border-danger border-opacity-25 py-3 px-4 mt-2" style="max-height: 200px; overflow-y: auto;">
                        <ul class="list-unstyled mb-0">
                            @foreach(session('delete_success')['items'] as $idx => $g)
                                <li class="py-1 border-bottom border-gray-200 d-flex justify-content-between align-items-center fs-7">
                                    <span class="text-gray-800 fw-bold">{{ $idx + 1 }}. {{ $g['nama'] }}</span>
                                    <span class="badge badge-light-danger">{{ $g['code'] }}</span>
                                </li>
                            @endforeach
                        </ul>
                        @if(session('delete_success')['count'] > count(session('delete_success')['items']))
                            <div class="text-center text-muted fs-8 pt-2">
                                ... dan <strong>{{ session('delete_success')['count'] - count(session('delete_success')['items']) }}</strong> guru lainnya telah dihapus.
                            </div>
                        @endif
                    </div>
                </div>
